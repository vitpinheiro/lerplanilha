...
# Função responsável pela leitura dos arquivos em formato ".xls"
...
def page_internacao():
    ...
    if arquivo_demonstrativo is not None:
        ...
        df_demonstrativo = df_demonstrativo[nomes_colunas_guias].astype(str)
        df_demonstrativo = df_demonstrativo.rename(columns={'Guia': 'GUIA_ATENDIMENTO', 'Dt item': 'DATA_GUIAS'})

        if not df_demonstrativo.empty:
    ...
    if arquivo_atendimentos is not None:
        df_atendimentos = pd.read_excel(arquivo_atendimentos).astype(str)

        if 'df_demonstrativo' in locals() and not df_demonstrativo.empty:
            # Realizar o merge dos DataFrames, incluindo todos os registros
            merge_df = pd.merge(df_demonstrativo, df_atendimentos, on='GUIA_ATENDIMENTO', how='outer', indicator=True)
            
            # Adicionar uma coluna para identificar se a guia foi encontrada ou não
            merge_df['STATUS'] = merge_df['_merge'].map({'left_only': 'Não encontrado', 'both': 'Encontrado', 'right_only': 'Não encontrado'})

            # Remover a coluna '_merge' pois não é necessária para o usuário
            merge_df.drop(columns=['_merge'], inplace=True)

            # Exibir o DataFrame completo
            st.dataframe(merge_df)

            # Criar um "buffer de memória" que pode ser utilizado para leitura de dados binários
            output2 = BytesIO()

            # Transforma o dataframe resultado em formato excel, desconsiderando os índices
            merge_df.to_excel(output2, index=False)
            ...
            st.write("Por favor, faça o upload do arquivo 'Demonstrativo' primeiro!")
